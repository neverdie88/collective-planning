...
class Model(object):
    def __init__(self, actor, critic, observation_space, action_space, num_procs,
        ...
        action_count = []
        ADV = tf.placeholder(tf.float32, shape=(None,),name=('adv'))
        R = []
        ...
        pg_loss = -tf.reduce_mean(
            ADV * tf.reduce_sum(action_count * tf.log(actor_tf + TINY), axis=(1, 2))) #- ent_coef * entropy

        vf_loss = tf.reduce_mean(tf.square(tf.reduce_sum((critic_tf- R) * action_count, axis=(1, 2))))
        if critic_l2_reg > 0:
...
class Runner(object):
    ...
    def run(self):
        ...
        mb_advantages = np.sum(mb_values * action_count, axis=(1, 2)) / self.population
...
def main():
    ...
    def make_env(rank):
        def env_fn():
            env = CGMgrid(population, edge1, edge2)
            env.seed(seed + rank)
            if rank == 0:

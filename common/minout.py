...
def minout(inputs, num_units, axis=-1, name=None):
  ...
  "Maxout Networks" Ian J. Goodfellow, David Warde-Farley, Mehdi Mirza, Aaron Courville,
   ...
   Arguments:
   inputs: Tensor input
   num_units: Specifies how many features will remain after minout in the `axis` dimension
  ...
  """
  return MinOut(num_units=num_units, axis=axis, name=name)(inputs)


class MinOut(base.Layer):
  """Adds a maxout op from https://arxiv.org/abs/1302.4389
  ...
  def call(self, inputs):
    ...
    outputs = math_ops.reduce_min(gen_array_ops.reshape(inputs, shape), -1, keep_dims=False)

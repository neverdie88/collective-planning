...
class Model(object):
    def __init__(self, actor, critic, observation_space, action_space, num_procs,
        ...
        pg_loss = -tf.reduce_mean(
            critic_tf * tf.reduce_sum(action_count * tf.log(actor_tf + TINY), axis=(1, 2)))/(population)# - 0.01*entropy
        ...
        def get_next_state_penalty(obs, action_count_samples):
            td_map = {obs0: obs, action_count: action_count_samples}
            next_penalty_P = sess.run(
                future_penalty_tf,
                td_map
            )
            return next_penalty_P

        def get_next_state_revenue(obs, action_count_samples):
            td_map = {obs0: obs, action_count: action_count_samples}
        ...
        def get_immediate_revenues(obs, action_count_samples):
            td_map = {obs0: obs, action_count: action_count_samples}
            next_state_revenue = sess.run(
                immediateRevenues_tf,
                td_map
            )
            return next_state_revenue
        ...
        self.get_next_state_penalty = get_next_state_penalty
        self.get_next_state_revenue = get_next_state_revenue
        self.get_immediate_revenues = get_immediate_revenues
        self.get_next_state_count = get_next_state_count

        def train(obs, state_count_samples, action_count_samples, sample_r, sample_next_R, sample_next_P, vf_mean):
            td_map = {obs0: obs, state_count: state_count_samples, action_count: action_count_samples, r: sample_r,
                      next_R: sample_next_R, next_P: sample_next_P, vf_mean_tf: vf_mean}
        ...
        def trainCritic(obs, state_count_samples, action_count_samples, sample_r, sample_next_R, sample_next_P):
            td_map = {obs0: obs, state_count: state_count_samples, action_count: action_count_samples, r: sample_r,
                      next_R: sample_next_R, next_P: sample_next_P}
            value_loss, _, immediate_val_loss, revenue_VF_loss, penalty_VF_loss = sess.run(
                [vf_loss, pre_critic_update, vf_immediate_loss, vf_revenue_VF_loss, vf_penalty_VF_loss],
                td_map
            )
...
class Runner(object):
    ...
    def run(self):
        ...
        mb_penalties = mb_penalties.reshape(self.batch_inflow_shape)

        if self.vf_normalization:
            if self.epoch == 0:
                # Thien check dimension here
                self.meanAds = np.sum(mb_trip_pays + mb_penalties + self.model.get_next_state_penalty(mb_obs,
                                                                                                      mb_actions) + self.model.get_next_state_revenue(
                    mb_obs, mb_actions)) / len(mb_trip_pays)
                self.epoch = 1
            else:
                self.meanAds1 = np.sum(mb_trip_pays + mb_penalties + self.model.get_next_state_penalty(mb_obs,
                                                                                                       mb_actions) + self.model.get_next_state_revenue(
                    mb_obs, mb_actions)) / len(mb_trip_pays)
                self.meanAds = 0.9 * self.meanAds1 + 0.1 * self.meanAds
        return mb_obs, mb_actions, mb_states, mb_action_probs, mb_trip_pays, mb_next_penalty_returns, mb_next_trip_pay_returns, average_reward, average_penalty, average_revenue,  self.meanAds
...
def learn(actor, critic, env, seed, nsteps=5, nstack=4, total_timesteps=int(80e6), ent_coef=0.01, gamma=0.99,
    ...
    for k in range(100):
        ...
        obs, actions, states, action_probs, mb_trip_pays, mb_next_penalty_returns, mb_next_trip_pay_returns, average_reward, average_penalty, average_revenue, vf_mean = runner.run()
    ...
    for update in range(1, 20101):
        ...
        if update % 1000 == 0:
            if average_running_reward>best_value:
                ...
                model.save(
                    './TaxiVanilla-CAC' + norm_type + '.criticTraining' + str(
                    ...
                    'TaxiVanilla-CAC' + norm_type + '.param',
                    checkPoint=update)
        if update %100 == 0:
            average_running_reward = 0
    env.close()

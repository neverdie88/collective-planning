...
class Model(object):
    def __init__(self, actor, critic, observation_space, action_space, num_procs,
        ...
        pg_loss = -tf.reduce_mean(
            tf.reduce_sum(ADV * action_count * tf.log(actor_tf + TINY), axis=(1, 2)))/population#-ent_coef*entropy/population
...
class Runner(object):
    ...
    def run(self):
        ...
        action_grads = self.model.get_action_grads(mb_obs, mb_actions)[0]
       ...
       # print('max grad, min grad:'+ str(max_grad) +',' + str(min_grad))
        ...
        if update % 1000 == 0:
            if average_running_reward > best_value:
                ...
                model.save('./TaxiFCCAC'+norm_type+'.criticTraining'+str(critic_training_type) +'.lr'+str(actor_lr)+'-'+str(critic_lr)+'.satisfied_percentage'+str(satisfied_percentage)+'.max_var'+str(max_var)+'.penalty_weight'+str(penalty_weight)+'/','TaxiFCCAC'+norm_type+'.param', checkPoint=update)
        if update % 100 == 0:
            average_running_reward = 0
    env.close()

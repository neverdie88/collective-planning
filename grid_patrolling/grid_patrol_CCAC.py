...
class Model(object):
    def __init__(self, actor, critic, observation_space, action_space, num_procs,
        ...
        pg_loss = -tf.reduce_mean(
            tf.reduce_sum(ADV * action_count * tf.log(actor_tf + TINY), axis=(1, 2)))/population# - 0.01*entropy/population
...
def learn(actor, critic, env, seed, nsteps=5, nstack=4, total_timesteps=int(80e6), ent_coef=0.01, gamma=0.9,
    ...
    # log_name = 'ApproxCOMA' + '.lr' + str(actor_lr) + '-' + str(critic_lr) + '.victimNum' + str(victimNum) + '.N' + str(N) + '.txt'
    ...
    for update in range(1, 20205):
        ...
        if update % log_interval == 0:
            ...
            if update % 10000 == 0:
                ...
                model.save('./GridPatrollingCOMA' + norm_type + '.criticTraining' + str(
                    critic_training_type) + '.lr' + str(actor_lr) + '-' + str(critic_lr) + '.victimNum' + str(
                    victimNum) + '.N' + str(N) + '/',
                           'FCOMA' + norm_type + '.param',
                           checkPoint=update)
            average_running_reward = 0
            average_running_value_loss = 0
            average_running_immediate_val_loss = 0

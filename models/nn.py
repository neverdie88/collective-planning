...
def normalizedDense(x, num_units, nonlinearity=None):
    ''' fully connected layer '''
    name = get_name('normalized_dense', counters)
    with tf.variable_scope(name):
        if init:
            # data based initialization of parameters
            ...
            V,b = get_vars_maybe_avg(['V','b'], ema)
            tf.assert_variables_initialized([V,b])
